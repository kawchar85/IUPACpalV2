...
string formatedTime(double duration) {
    ...
    if (duration < 1000) {
        ...
        ex = 0.01 + ex / 3;
        time_taken = static_cast<double>(duration+ex);
    } else if (duration < 1000000) {
        units = "us";
        time_taken = static_cast<double>(duration) / 1000.0;
    } else if (duration < 1000000000) {
        units = "ms";
        time_taken = static_cast<double>(duration) / 1000000.0;
    } else if (duration < 60000000000) {
        units = "s";
        time_taken = static_cast<double>(duration) / 1000000000.0;
    } else if (duration < 3600000000000) {
        units = "min";
        time_taken = static_cast<double>(duration) / 60000000000.0;
    } else if (duration < 86400000000000) {
        units = "h";
        time_taken = static_cast<double>(duration) / 3600000000000.0;
    } else {
        units = "day";
        time_taken = static_cast<double>(duration) / 86400000000000.0;
    }

    return (to_string(time_taken) + " " + units);
}
...
;
    ...
    // if (min_len < 2) { usage(); cout << "Error: min_len must not be less than 2." << endl; return 0; }
    // if (min_len > INT_MAX) { usage(); cout << "Error: min_len must not greater than " << INT_MAX << "." << endl; return 0; }
    // if (max_len < 0) { usage(); cout << "Error: max_len must not be a negative value." << endl; return 0; }
    ...
    vector<int> p = build_suffix_array(temp_s);
    ...
    file << "Number of mismatches allowed in IR: " << mismatches << endl;
    file << endl << endl << endl;
    file << "Inverted Repeats:" << endl;
            ...
            if (prev_left != left) {
                ...
                while (it != prev(palindromes.begin()) and get<0>(*it) == prev_left) {
                    ...
                    for (int i = 0; i < (inner_left - outer_left + 1); ++i) {
                        if( !MatchMatrix::match(seq[ outer_left - 1 + i ], complement[ seq[ outer_right - 1 - i ] ]) ) {
                            mis++;
                        }
                        len++;
